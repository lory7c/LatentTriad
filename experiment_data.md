...
| Pass A (decl-only) + Pass B (complete prompt, same as v1). risk = max( |  |  |  |  |  |  |  |
...
| Pass A (decl-only, ~50ms): prompt 截断在声明末尾 → decl-bnd → Decl-Probe → sc | 50 | 50 | 100% |  | 说明 |  |  |
| Pass B (完整 prompt, ~1945ms): 同 v1 → oper+bnd 几何特征 → PCA(8)+LR → score_ | 50 | 22 | 44% | Decl-Probe L31信号不够强 | 单趟完整prompt。SMP攻击全是操作侧——v1已足够 |  |  |
| risk = max(score_v1, score_decl)   attribution = argmax → DECL/OPER/BO | 50 | 50 | 100% | v1 完美检出 | 声明侧无攻击信号——SMP没有声明投毒。随机水平 |  |  |
...
| 效果: 几何特征反映 skill 内容而非 token 位置 → 跨分布鲁棒。Decoy AUROC +43pp, InvDecoy +59 |  |  |  |  |  |  |  |
| 签名: oper+bnd → center(-PC1) → 5几何特征 → PCA(8) → LR(C=0.01) · L7+ | AUROC | F1% | FPR% | FNR% | 层 | 说明 |  |
...
| if score_boundary < 0.15: risk = score_boundary  (模型确定安全→信任模型判断) | 1.000 | 100.0 | 0.0 | 0.0 | — | 全覆盖。max()取两路最强信号——decl-poison被Decl-Probe检出，oper-poison被Code-Probe检出 |  |
| else: risk = score_v3  (模型不确定→用 v3 代码级检测) |  |  |  |  |  |  |  |
| Boundary 在安全样本上天然极低 (~0.005)，在恶意样本上天然高 (~0.9)。Gate=0.15 无需调参。 |  |  |  |  |  |  |  |
| 效果: Lexical Decoy FPR 100%→0%, InvDecoy FPR 100%→0%。两张 Decoy 全部解决。 | 0.427 | 0.998 |  |  |  |  |  |
| 签名: oper+bnd → center(−PC1) → 5几何特征 → PCA(8) → LR(C=0.01) · L7+ |  |  |  |  |  |  |  |
...
| 论文策略：v1是核心方法（轻量、高性能），v2是完整版（补盲区+归因）。两者在现有benchmark上性能相同，v2多覆盖一种攻击类型。 |  |  |  |  |  |  |  |
...
| 关键模式：当数据集没有声明投毒攻击时，Decl-Probe输出随机(InvDecoy,SMP)或反映词法混杂(Decoy,MSB)。v2架构 | 0.964 | 0.996 |  |  |  |  |  |
...
| Paired CF, 484 samples. L7+ layer selection (L3 excluded). No interact |  |  |  |  |  |  |  |  |  |
| Method | AUROC | FPR% | FNR% | F1% | Precision% | Recall% | Train AUC | Notes |  |
| Static Regex | 0.540 | 100 | 0 | 66.7 | 50 | 100 | — | Always fires |  |
| TF-IDF + LR | 0.812 | 19.4 | 3.200 | 89.6 | 83.3 | 96.8 | 0.679 | C=0.01, unique prompt |  |
| LLM-as-judge | 0.630 | 100 | 0 | 66.7 | 50 | 100 | — | Mistral-7B zero-shot: predicts almost all MAL (FPR=100%, FNR=0%) |  |
...
| RouteGuard | 0.824 | 48.4 | 8.100 | 76.5 | 65.5 | 91.7 | 0.938 | Done: attn+rhid. Honest: AUC=0.824 on paired CF |  |
| Relational (v1) | 0.761 | 50 | 6.500 | 76.8 | 65.2 | 93.5 | 0.893 | L31, interact+PCA64 |  |
...
| AgentLens | 0.990 | 3.600 | 2.100 | 97.2 | 96.5 | 97.9 | 0.995 | L19 (L7+ verified, source confound) |
| RouteGuard | 0.795 | 47.7 | 11.1 | 74.5 | 64.1 | 88.9 | 0.796 | rhid only. Honest: not inflated by source confound (unlike others at 0 |
| Relational (v1) | 0.992 | 2.900 | 6 | 95.5 | 97 | 94 | 0.993 | L11 |
| Centered Relational | 0.991 | 7.800 | 3 | 94.8 | 92.7 | 97 | 0.991 | L11 |
...
| RouteGuard | 0.998 | 0 | 1 | 99.5 | 100 | 99 | 1 | 6K in-domain, rhid only (pre-L7+. Needs MSB route_hidden re-extract fo |
| Relational (v1) | 1 | 0 | 0.700 | 99.6 | 100 | 99.3 | 1 | 4K in-domain, pre-L7+ |
| Centered Relational | 1 | 0 | 1.700 | 99.1 | 100 | 98.3 | 1 | 4K in-domain, pre-L7+ |
...
| Relational (v1) | 0.460 | 100 | 0 | 66.7 | 50 | 100 | — | Obsolete (interaction+PCA64). FPR=100% confirms noise |  |
| Centered Relational | 0.705 | 57.3 | 14.7 | 70.3 | 59.2 | 85.3 | — | Centered (-1PC). AUC=0.71 better than SkillProbe on decoy—centering he |  |
...
| AgentLens | 0.676 | 78.1 | 0.600 | 71.6 | 53.2 | 99.4 | 0.989 | L15. Catches 99% malicious but FPR=78% |  |
| RouteGuard | 0.500 | 100 | 0 | 66.7 | 50 | 100 | 0.734 | rhid only. Random on clean vocab. Cross-extract. |  |
| Relational (v1) | 0.824 | 100 | 0 | 66.7 | 50 | 100 | — | AUC=0.82 ranks correctly but FPR=100% (threshold issue) |  |
| Centered Relational | 0.846 | 21.9 | 1.300 | 89.5 | 80.9 | 98.7 | — | Centered (-1PC). AUC=0.85 BEST on InvDecoy! Centering removes anisotro |  |
